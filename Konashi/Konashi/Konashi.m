...
{
    KonashiResult result = KonashiResultFailed;
    if (pin >= KonashiAnalogIO0 && pin <= KonashiAnalogIO2 && milliVolt >= 0 && milliVolt <= KonashiAnalogReference && activePeripheral && activePeripheral.state == CBPeripheralStateConnected) {
...
{
    KonashiResult result = KonashiResultFailed;
    if ((mode == KonashiI2CModeDisable || mode == KonashiI2CModeEnable || mode == KonashiI2CModeEnable100K || mode == KonashiI2CModeEnable400K) && activePeripheral && activePeripheral.state == CBPeripheralStateConnected) {
        i2cSetting = mode;
...
- (KonashiResult)i2cWrite:(int)length data:(unsigned char*)data address:(unsigned char)address
{
    ...
    KonashiResult result = KonashiResultFailed;
    if (length > 0 && (i2cSetting == KonashiI2CModeEnable || i2cSetting == KonashiI2CModeEnable100K || i2cSetting == KonashiI2CModeEnable400K) && activePeripheral && activePeripheral.state == CBPeripheralStateConnected) {
        t[0] = length + 1;
...
{
    KonashiResult result = KonashiResultFailed;
    if (activePeripheral && activePeripheral.state == CBPeripheralStateConnected && (mode == KonashiUartModeDisable || mode == KonashiUartModeEnable)) {
        Byte t = mode;
        NSData* d = [[NSData alloc] initWithBytes:&t length:1];
...
- (KonashiResult)setUartBaudrate:(KonashiUartRate)baudrate
{
    ...
    if (activePeripheral && activePeripheral.state == CBPeripheralStateConnected && uartSetting == KonashiUartModeDisable) {
        if (baudrate == KonashiUartRate2K4 || baudrate == KonashiUartRate9K6) {
...
{
    if ((condition == KonashiI2CConditionStart || condition == KonashiI2CConditionRestart || condition == KonashiI2CConditionStop) && activePeripheral && activePeripheral.state == CBPeripheralStateConnected) {
        Byte t = condition;
        NSData* d = [[NSData alloc] initWithBytes:&t length:1];
